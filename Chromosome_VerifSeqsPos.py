...
def GetDirection(Chrom, Beg, End):
	...
	if Chrom[Beg:Beg+3] == "ATG" and Chrom[End:End+3] in ListeStops:
		...
		#print "**********"
		Strend = "+"
		SeqBefore = Chrom[Beg-100:Beg]
		SeqAfter = Chrom[End:End+100]
	else:
		...
		if Chrom[Beg-3:Beg] in ListeReverseStop and Chrom[End-3:End] == "CAT":
			...
			#print "***********"
			Strend = "-"
			SeqBefore = Chrom[End:End+100]
			SeqAfter = Chrom[Beg-100:Beg]
		else:
			Strend = "w"
			SeqBefore = Chrom[End:End+100]
			SeqAfter = Chrom[Beg-100:Beg]
...
def VerifSeq(Seq, Strend, LeChrom, Beg, End):
	Seq = UpperSeqs(Seq)
	if Strend == "+":
		ChromSeq = LeChrom[Beg:End]
		print ChromSeq
		print Seq
		#print "*****************"
		if len(ChromSeq) == len(Seq):
			if ChromSeq == Seq:
				Correct = "ForwardGood"
				#print Correct
			else:
				Correct = "ForwardSameLenWrongSeq"
				#print Seq
				#print ChromSeq
				#print "******************************"
		else:
			Correct = "ForwardWrongSize"
	if Strend == "-":
		ChromSeqFirst = LeChrom[Beg:End]
		ChromSeq = ReverseTrans(ChromSeqFirst)
		#print len(ChromSeq)
		#print len(Seq)
		#print "********************"
		if len(ChromSeq) == len(Seq):
			if ChromSeq == Seq:
                                ...
                                Correct = "ReverseSameLenWrongSeq"
                                #print Seq
                                #print ChromSeq
                                #print "******************************"
		else:
			Correct = "ReverseWrongSize"
	return Correct
...
def SearchORF(File, C1, C2, C3, C4, C5, C6, C7, C8, C9, C10, C11, C12, C13, C14, C15, C16, C17, C18, C19, C20, C21, C22, CX, CY, DicoSeq):
	...
	for i in File:
		...
		if Chrom in DicoChrom.keys():
			...
			if Strend == "w":
				...
				Seq = DicoSeq[NameSeq]
				Status = VerifSeq(Seq, Strend, LeChrom, Beg, End)
				if Status == "ReverseWrongSize" or Status == "ForwardWrongSize":
					NbNotGoodSize += 1
				elif Status == "ForwardSameLenWrongSeq" or Status == "ReverseSameLenWrongSeq":
					NbNotGoodSeq += 1
				elif Status == "ForwardGood" or Status == "ReverseGood":
					NbGoodSeq+=1
		else:
			NbWrongChrom += 1
	print ("Nb wrong chromosomes : "+str(NbWrongChrom))
	print ("Nb Not ATG : "+str(NbNotATG))
	print ("Nb Not Good Size : "+str(NbNotGoodSize))
	print ("Nb Not Good Seq : "+str(NbNotGoodSeq))
	print ("Nb Good : "+str(NbGoodSeq))
	#BuildFile("Sequences100BeforeDeNovoGene", ListeSeqBefore, Name)
	#BuildFile("Sequences100AfterDeNovoGene", ListeSeqAfter, Name)
	#BuildFile("Sequences100AfterBeforeDeNovoGenes", ListeSeqBeforeAfterW, NameW)
...
DicoSeq = GetSequencesSize("hsapiens.orfs.dna.fa")
Correspondance = openFile("CorrespondanceFinal1")
SearchORF(Correspondance, C1, C2, C3, C4, C5, C6, C7, C8, C9, C10, C11, C12, C13, C14, C15, C16, C17, C18, C19, C20, C21, C22, CX, CY, DicoSeq)
