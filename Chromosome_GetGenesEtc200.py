...
def MakeFile(Dico, Genes, C1, C2, C3, C4, C5, C6, C7, C8, C9, C10, C11, C12, C13, C14, C15, C16, C17, C18, C19, C20, C21, C22, CX, CY):
	...
	for i in Genes:
		...
		if ligne[0] in Dico.keys():
			...
			if Direction == "+":
				Before = LeChrom[Beg-200:Beg]
				After = LeChrom[End:End+200]
			else:
				Before = LeChrom[End:End+200]
				After = LeChrom[Beg-200:Beg]
	...
	F = open("Genes200Before", "w")
	for i in DicoGenesBefore.keys():
		F.write(">"+i+"\n")
		F.write(DicoGenesBefore[i]+"\n")
	F.close()
	F = open("Genes200After", "w")
	for i in DicoGenesAfter.keys():
		F.write(">"+i+"\n")
		F.write(DicoGenesAfter[i]+"\n")
	F.close()
        F = open("Pseudogenes200Before", "w")
        for i in DicoPseudogenesBefore.keys():
                F.write(">"+i+"\n")
                F.write(DicoPseudogenesBefore[i]+"\n")
        F.close()
        F = open("Pseudogenes200After", "w")
        for i in DicoPseudogenesAfter.keys():
                F.write(">"+i+"\n")
                F.write(DicoPseudogenesAfter[i]+"\n")
        F.close()
        F.close()
        F = open("NcRNAgenes200Before", "w")
        for i in DicoNcRNABefore.keys():
                F.write(">"+i+"\n")
                F.write(DicoNcRNABefore[i]+"\n")
        F.close()
	F = open("NcRNAgenes200After", "w")
        for i in DicoNcRNAAfter.keys():
                F.write(">"+i+"\n")
                F.write(DicoNcRNAAfter[i]+"\n")
        F.close()
