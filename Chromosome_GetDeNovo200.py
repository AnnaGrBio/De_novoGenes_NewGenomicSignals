...
def SearchORF(D, C1, C2, C3, C4, C5, C6, C7, C8, C9, C10, C11, C12, C13, C14, C15, C16, C17, C18, C19, C20, C21, C22, CX, CY):
	...
	F = open("Sequences200BeforeDeNovoGene", "w")
	for i in ListeGood:
		F.write(">"+i)
	...
	for i in ListToKeep:
                ...
                F.write(DicoAllSeqBefore[i]+str("\n"))
	F.close()
	F = open("Sequences200AfterDeNovoGene", "w")
        for i in ListeGood:
                F.write(">"+i)
        ...
        for i in ListToKeep:
                ...
                F.write(DicoAllSeqAfter[i]+str("\n"))
        F.close()
	F = open("ListeDeNovoDoublon_SameTranscript", "w")
	for i in ListeDoublons:
		for j in i:
			F.write(j+" ")
		F.write("\n")
	F.close()
	F = open("SpecialCorres200", "w")
	for i in ListeGood:
		F.write(i)
	for i in ListToKeep:
		F.write(i)
	...
	for i in File:
		...
		if Elts[8] == "+" or Elts[8] == "-":
			...
			if Elts[8] == "+":
				...
				IntervalBegin = [Begin-200, Begin]
				IntervalEnd = [End, End+200]
				D[i] = [Chrom, IntervalBegin, IntervalEnd]
			else:
				Begin = int(Elts[3])+int(Elts[9])
				End = int(Elts[2])-int(Elts[10])
				IntervalBegin = [Begin, Begin+200]
				IntervalEnd = [End-200, End]
				D[i] = [Chrom, IntervalBegin, IntervalEnd]
	return D

Correspondance = openFile("CorrespondanceFinal3")
